...
#
# Dieses Modulk importiert Daten aus dem umfangreichen OpenData-Angebot
...
def tageswerte_land(auswertungsland, still=False):
    ...
    assert auswertungsland in ['Baden-Württemberg', 'Bayern', 'Berlin', 'Brandenburg',
                    ...
                    'Schleswig-Holsetin', 'Thüringen'], 'Bitte Namem eines Bundeslands' \
...
def tagestemp_land(auswertungsland, still=False):  # aus Kompatibilitätsgründen noch drin
    """
    Parameters
    ----------
    land : Name of federal state (required)
    ...
    DWD_PFAD = 'https://opendata.dwd.de/climate_environment/CDC/' \
               'observations_germany/climate/daily/kl/'
    if auswertungsland not in ['Baden-Württemberg', 'Bayern', 'Berlin', 'Brandenburg',
                    'Bremen', 'Hamburg', 'Hessen', 'Mecklenburg-Vorpommern',
                    'Niedersachsen', 'Nordrhein-Westfalen', 'Rheinland-Pfalz',
                    'Saarland', 'Sachsen', 'Sachsen-Anhalt',
                    'Schleswig-Holsetin', 'Thüringen']:
        raise ValueError
    # %% Laden der Wetterstationen vom DWD-OpenData-Server. Infos, Datensatz-
    # beschreibung etc. hier: https://opendata.dwd.de/README.txt
    url = DWD_PFAD + 'historical/KL_Tageswerte_Beschreibung_Stationen.txt'
...
if __name__ == "__main__":
    ...
    print(f'\nJähliche Durchschnittstempertauren für {sys.argv[1]}.')
    print(tageswerte.groupby('Jahr')['Temp'].mean())
